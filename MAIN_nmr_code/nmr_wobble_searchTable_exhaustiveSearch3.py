...
Jun 15, 2020 CHeng: Find optimum from random search first, then exhaustive search
'''
...
en_remote_dbg = 0
fig_num = 1
en_fig = 0  # enable figure for every measurement
...
def readTable( minReflxTable_tmp ):
    optValue = 0
    cpar_i = 0
    cser_i = 0 
    for i in range( len( minReflxTable_tmp ) ):
        if ( optValue > minReflxTable_tmp[i, 1] ):
            optValue = minReflxTable_tmp[i, 1] 
            cpar_i = minReflxTable_tmp[i, 2]
            cser_i =minReflxTable_tmp[i, 3]
    return cpar_i, cser_i
